...
export default function Categories() {
  ...
  async function createCat(){
    ...
    const response = await axiosApi(`/category/`, {
      ...
      window.alert("Erro ao criar categoria! Verifique se você preencheu todos os campos");
    });
  ...
  async function deleteCat(){
    if(!confirm('Você tem certeza que quer deletar essa categoria ?'))
        return;
      ...
      <div className=" flex justify-center items-center w-full gap-5">
        ...
        { data?.map((cat) => (
           ...
           <SheetTrigger asChild>
             <Button variant="outline" onClick={() => setCatId(cat.id)}> {cat.name}</Button>
           </SheetTrigger>
           ...
           <SheetContent side="bottom">  
             ...
             <div className="grid gap-4 py-4">
               ...
               <div className="grid grid-cols-4 items-center gap-4">          
                 ...
                 <Input id="name" defaultValue={cat.name} className="col-span-3" onChange={handleChange} />
               </div>

               <div className="grid grid-cols-4 items-center gap-4">
                 <Label htmlFor="username" className="text-right"> Departamento </Label>

                 <Select defaultValue={""+cat.department_id} onValueChange={handleDepChange}>
                  <SelectTrigger className="w-[180px]"> <SelectValue placeholder="Selecione o Departamento" /> </SelectTrigger>
                  <SelectContent>
                    <SelectGroup>
                      <SelectLabel>Departamento</SelectLabel>
                      
                      { data_dep.data?.map((dep) => (
                        <SelectItem id={""+dep.id} value={""+dep.id}  key={""+dep.id} > {dep.name} </SelectItem>  
                      ))}
             ...
             <SheetFooter>
               <SheetClose asChild>
                 <Button type="button" onClick={updateCat}>Save changes</Button>
               </SheetClose>
               <SheetClose asChild>
                 <Button type="button" variant="destructive" onClick={deleteCat}>Delete</Button>
               </SheetClose>
             </SheetFooter>
           </SheetContent>
         </Sheet>
        ...
        <Sheet>
          ...
          <SheetContent side="top">  
            ...
            <SheetFooter>
              <SheetClose asChild>
                <Button type="button" onClick={createCat}>Save</Button>
              </SheetClose>
