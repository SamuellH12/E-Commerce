...
export async function emptyShoppingCart(req: Request, res: Response) {
  ...
  if (req.path == "/checkout") {
    res.send(
      "Shopping cart successfully checked out. Shopping cart is now empty"
    );
  } else {
...
export async function checkoutCart(req: Request, res: Response) {
  const { data: cart_products, error } = await supabase.from("shopping-cart")
    .select(`
        products(*),
        amount
      `);
  ...
  if (error) {
    ...
    for (const row of cart_products) {
      if (row.products.stock_quantity < row.amount) {
        res.send(
          "Failed to complete checkout, there are more units of " +
            row.products.name +
            " on cart than on stock"
        );
        return;
      }
    }
